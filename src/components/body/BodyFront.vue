 ...
 * @LastEditors: Sid Li
 * @LastEditTime: 2026-01-24 20:58:05
 * @FilePath: \electron-zxa\src\components\body\BodyFront.vue
...
const initPointTreat = () => {
  ...
  // 过滤bodyType=0的穴位（正面），如果为空则用原始数据
  pointTreat.value =
    pointDataCopy.value.filter((item) => item.bodyType === 0) || pointDataCopy.value;
...
const updatePointStatus = () => {
  ...
  nextTick(() => {
    ...
    console.log("正面最终渲染的穴位数据：", pointData.value);
  });
};
...
watch(
  () => [...props.newPlanPoint],
  (newVal) => {
    console.log("最新newPlanPoint分组数据（正面）：", newVal);
...
watch(
  () => pointData.value,
  (newVal) => {
    console.log("pointData最终渲染数据（bodyType=0）：", newVal);
...
onMounted(() => {
  console.log("BodyFront组件挂载（bodyType=0）");
  initPointTreat();
  updatePointStatus();
});
</script>
...
.bg-body-norem {
  width: 441px;
  height: 636px;
  background: url("@/assets/pic/body/body0.png") no-repeat center center;
